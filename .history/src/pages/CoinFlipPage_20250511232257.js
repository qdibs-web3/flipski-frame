...
const CoinFlipPage = () => {
  ...
  const getWalletClient = useCallback(async () => {
    // This check is still important as a safeguard within getWalletClient itself
    if (!activeWalletInstance || !walletAddress) {
      console.error("Active wallet instance or wallet address not available for getWalletClient.");
      setError(
  ...
  const handleDegen = async () => {
    ...
    // Enhanced check: require both walletAddress and activeWalletInstance
    if (!walletAddress || !activeWalletInstance) { 
      setError("Wallet not fully ready. Please ensure it's connected via the navigation bar and try again, or refresh the page.");
      setTimeout(() => setError(""), 4000); // Increased timeout for better visibility
      return;
    }
    ...
    // Call getWalletClient only after confirming activeWalletInstance is present
    const walletClient = await getWalletClient();
    if (!walletClient) { // This will now primarily catch errors from within getWalletClient if provider fails
        // setError is already handled by getWalletClient if it returns null
        return;
    }
