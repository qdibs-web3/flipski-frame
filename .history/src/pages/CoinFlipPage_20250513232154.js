    ...
    try {
      ...
      const history = logs
        ...
        .map((log, index) => {
          ...
          const mappedResult = rawResult === 0n ? "Heads" : "Tails";
          const payoutAmount = log.args.payoutAmount;
          const won = payoutAmount > 0n;
          console.log(`HISTORY_DEBUG: Log ${index} - GameID: ${log.args.gameId.toString()}, Raw Result: ${rawResult}, Mapped Result: ${mappedResult}, Payout: ${formatEther(payoutAmount)}, Won: ${won}`);
