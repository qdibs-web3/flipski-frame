...
const CoinFlipPage = () => {
  const {
    walletAddress,
    // connectWallet, // Removed as button is removed from this page
    // isConnecting, // Removed as button is removed from this page
    userRelatedError, 
  } = useWallet();
  ...
  const getWalletClient = useCallback(async () => {
    if (!activeWallet || !walletAddress) {
      console.error("Active wallet or wallet address not available.");
      setError(
    ...
    try {
      ...
      if (!provider) {
        console.error("Failed to get provider from active wallet.");
  ...
  useEffect(() => {
    if (walletAddress) { // Fetch balance only if wallet is connected
        fetchEthBalance();
    }
  }, [walletAddress, fetchEthBalance]); // Added walletAddress dependency

  useEffect(() => {
    if (walletAddress) { // Fetch history only if wallet is connected
        fetchGameHistory();
        const interval = setInterval(fetchGameHistory, 30000); 
        return () => clearInterval(interval);
    }
  }, [walletAddress, fetchGameHistory]); // Added walletAddress dependency
  ...
  const handleDegen = async () => {
    ...
    if (!walletAddress) {
      setError("Connect wallet first (from the navigation bar)."); // Updated message
