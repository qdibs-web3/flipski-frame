...
const CoinFlipPage = () => {
  const {
    walletAddress,
    userRelatedError,
    activeWalletInstance, // Still needed for getWalletClient
    isConnecting,         // For button text/disabled state
    isConnected,          // Now using this for primary readiness check
  } = useWallet();
  ...
  const handleDegen = async () => {
    ...
    // Use isConnected from context for a more reliable check
    if (!isConnected) { 
    ...
    if (!walletClient) {
        // setError is handled by getWalletClient if it returns null
        return;
    }
