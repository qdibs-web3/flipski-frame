...
// CORRECTED ABI IMPORT PATH BELOW
...
const CoinFlipPage = () => {
    ...
    try {
      ...
      const history = logs
        ...
        .map((log) => ({
          ...
          result: log.args.result === 0n ? "Heads" : "Tails", // Correctly maps 0 to Heads, 1 to Tails
  ...
  useEffect(() => {
    if (currentFlipAttempt && currentFlipAttempt.gameId && gameHistory.length > 0) {
      ...
      if (settledGame) {
        ...
        const actualSide = settledGame.result.toLowerCase(); // Should be 'heads' or 'tails'
        setFlipResult({
  ...
  const handleDegen = async () => {
    ...
    const choiceAsNumber = selectedSide === "heads" ? 0 : 1; // Correct: Heads (FLIP) = 0, Tails (SKI) = 1
