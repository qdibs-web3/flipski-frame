...
const CoinFlipPage = () => {
  ...
  const handleDegen = async () => {
    ...
    try {
      ...
      console.log("DEBUG: Flip request transaction sent:", flipTxHash);
      const requestReceipt = await publicClient.waitForTransactionReceipt({ hash: flipTxHash });
      console.log("DEBUG: Flip request receipt received:", requestReceipt);
      ...
      if (!gameRequestedEventAbi) {
        console.error("DEBUG: GameRequested event ABI definition not found!");
      } else {
        console.log("DEBUG: GameRequested event ABI definition found.");
      }

      console.log("DEBUG: Iterating through receipt logs. Total logs:", requestReceipt.logs.length);
      for (const i in requestReceipt.logs) {
        const log = requestReceipt.logs[i];
        console.log(`DEBUG: Processing log index ${i}:`, log);
        try {
          const decodedLog = decodeEventLog({ abi: FlipSkiBaseVRFABI.abi, data: log.data, topics: log.topics });
          console.log(`DEBUG: Decoded log index ${i}:`, decodedLog);

          if (decodedLog && decodedLog.eventName === "GameRequested") {
            console.log(`DEBUG: GameRequested event found at index ${i}. Args:`, decodedLog.args);
            if (decodedLog.args.player && decodedLog.args.player.toLowerCase() === walletAddress.toLowerCase()) {
              console.log(`DEBUG: Player address matches for GameRequested event at index ${i}.`);
              parsedGameRequested = {
              ...
              console.log(`DEBUG: Player address mismatch for GameRequested event at index ${i}. Expected: ${walletAddress}, Got: ${decodedLog.args.player}`);
            }
          } else if (decodedLog) {
            console.log(`DEBUG: Decoded log at index ${i} is not GameRequested. Event name: ${decodedLog.eventName}`);
          } else {
            console.log(`DEBUG: Could not decode log at index ${i}.`);
          }
        } catch (e) {
          console.error(`DEBUG: Error decoding log at index ${i}:`, e, "Log data:", log);
        }
      }

      if (parsedGameRequested) {
        console.log("DEBUG: GameRequested event successfully parsed:", parsedGameRequested);
        setCurrentFlipAttempt(parsedGameRequested);
        setIsFlipping(true);
        setError("");
      } else {
        console.error("DEBUG: Could not parse GameRequested event from receipt after iterating all logs.", requestReceipt);
