...
const CoinFlipPage = () => {
  const {
    ...
    isConnecting,
    // isConnected, // Removed isConnected from destructuring as we'll use walletAddress directly
  } = useWallet();
  ...
  const handleDegen = async () => {
    ...
    // Use walletAddress for the initial check as requested by user
    if (!walletAddress) { 
...
export default CoinFlipPage.js;
