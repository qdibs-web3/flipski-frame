...
const LevelSystem = ({ walletAddress, gameResult }) => {
  ...
  return (
    <div className="level-system">
      ...
      // Inside your component, replace the progress bar section with this:
        <div className="progress-container">
        <div className="progress-bar">
            <div 
            className="progress-fill" 
            style={{ 
                width: '50%', // Hardcoded 50% to test if it appears at all
                backgroundColor: '#39ff14' 
            }}
            ></div>
        </div>
        <div className="progress-labels">
            <span className="progress-label">{playerData.level}</span>
            <span className="progress-label">{playerData.level < 100 ? playerData.level + 1 : 'MAX'}</span>
        </div>
        </div>
