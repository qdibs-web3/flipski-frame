...
const CoinFlipPage = () => {
  ...
  const getWalletClient = useCallback(async () => {
    ...
    // In mini app mode, use window.ethereum as fallback since Farcaster provides it
    if (isMiniApp) {
      console.log("CoinFlipPage: Mini app mode - using window.ethereum provider");
      try {
        // In Farcaster mini app, window.ethereum should be available and connected
        if (typeof window !== 'undefined' && window.ethereum) {
          console.log("CoinFlipPage: Found window.ethereum in mini app");
        ...
        throw new Error("window.ethereum not available in mini app");
      } catch (err) {
        console.error("CoinFlipPage: Error creating mini app wallet client:", err);
  ...
  return (
    <div className="coinflip-container">
      ...
      {walletAddress && (
        ...
        {walletAddress && (
          <div className={`wallet-info-active ${isMiniApp ? 'mini-app-mode' : ''}`}
               style={isMiniApp ? {
                 width: 'calc(35% - 8px)',
                 maxWidth: '140px',
                 padding: '6px 8px',
            ...
            <p style={{ fontSize: isMiniApp ? '9px' : '14px', margin: isMiniApp ? '1px 0' : '2px 0' }}>
              Wallet: <span className="wallet-address">{walletAddress.slice(0, 4)}...{walletAddress.slice(-3)}</span>
            </p>
            <p style={{ fontSize: isMiniApp ? '9px' : '14px', margin: isMiniApp ? '1px 0' : '2px 0' }}>
              Balance: <span className="balance-info">{parseFloat(ethBalance).toFixed(3)} ETH</span>
            </p>
          </div>
        )}
              ...
              {gameHistory.map((game) => (
                <li key={game.gameId} className={game.won ? "win-history" : "loss-history"}>
                  ...
                  {game.fulfillmentTxHash && (
                    <a href={`https://basescan.org/tx/${game.fulfillmentTxHash}`} target="_blank" rel="noopener noreferrer" className="history-tx-link">
                      (View VRF Tx)
                    </a>
                  )}
                </li>
