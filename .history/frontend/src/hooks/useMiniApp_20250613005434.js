// Enhanced useMiniApp hook with mobile WebView debugging
...
export const useMiniApp = () => {
  ...
  useEffect(() => {
    const detectEnvironment = async () => {
      try {
        ...
        // MOBILE DEBUG: Add environment info
        ...
        // MOBILE DEBUG: Enhanced logging
        addDebug(`🔍 Quick checks: UA=${hasFarcasterUA}, Frame=${isInFrame}, Params=${hasFarcasterParams}`);
        addDebug(`📱 Frame check details: parent=${window.parent !== window}, element=${window.frameElement !== null}`);
        ...
        if (hasFarcasterUA || hasFarcasterParams || isInFrame) {
          try {
            ...
            const timeoutPromise = new Promise((_, reject) => 
              setTimeout(() => reject(new Error('SDK load timeout')), 15000) // 15 seconds for mobile
            );
            ...
            const detectionTimeoutPromise = new Promise((_, reject) => 
              setTimeout(() => reject(new Error('Detection timeout')), 12000) // 12 seconds for detection
            );
    ...
    if (isMiniApp && !isReady) {
      ...
      if (!workingSdk) {
        try {
          ...
          const timeoutPromise = new Promise((_, reject) => 
            setTimeout(() => reject(new Error('SDK load timeout')), 8000)
          );
  ...
  useEffect(() => {
    ...
    }, 25000); // 25 seconds emergency timeout
